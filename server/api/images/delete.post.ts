...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const body = await readBodySafe<{ paths?: string[] }>(event)
    const paths = body?.paths
    if (!Array.isArray(paths)) return fail(event, 400, 40001, '参数错误')
    ...
    if (!enableLocalStorage && enabledNodes.length > 0) {
      ...
      for (const node of enabledNodes) {
        ...
        const bodyOut = { paths: paths.map((p) => joinNodePath(rootPath, p)) }
